...
const App: React.FC = () => {
  ...
  // Check for direct TV view via URL param ?view=tv
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Listen for storage changes from other tabs (secretariaat saves)
    const handleStorageChange = (e: StorageEvent) => {
        if (e.key?.startsWith('school_')) {
            loadAllData();
        }
    };
  ...
  const handleDeleteSick = useCallback((idToDelete: string) => {
    ...
    setSickEntries(prev => {
        ...
        StorageService.saveSickEntries(filtered);
        return filtered;
    });
    ...
    if (targetEntry) {
        setReplacements(prev => {
            ...
            StorageService.saveReplacements(filtered);
            return filtered;
        });
    }
  }, [sickEntries]);
    ...
    return (
      <div className="relative">
        ...
        <button 
          ...
          className="fixed bottom-0 right-0 opacity-0 hover:opacity-50 p-4 text-white bg-red-600 rounded-tl-xl transition z-[9999] no-print"
        >
          Exit TV
        </button>
      </div>
        ...
        <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-md border border-slate-200">
          ...
          <div className="space-y-6">
            <button 
                ...
                className="w-full py-4 bg-slate-800 text-white rounded-xl font-bold hover:bg-slate-900 transition-all flex items-center justify-center gap-3 shadow-lg"
            >
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans">
      <nav className="bg-slate-900 text-white shadow-2xl sticky top-0 z-50 border-b border-white/10 no-print">
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-2">
                <button 
                    ...
                    className="w-10 h-10 flex items-center justify-center rounded-xl bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700 transition-all" 
                    title="TV Modus"
                >
                    <Monitor size={20} />
                </button>
      ...
      <main className={`flex-1 w-full mx-auto p-8 no-print ${viewMode === 'PROCESSING' || viewMode === 'ADMIN' || viewMode === 'DASHBOARD' ? 'max-w-full' : 'max-w-7xl'}`}>
        {!isLoaded ? (
            ...
            <div className="animate-in fade-in duration-500">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        <div className="lg:col-span-4 space-y-6">
                            ...
                            <div className="bg-slate-900 p-8 rounded-3xl shadow-xl text-white">
                                <h4 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Systeem Status</h4>
                                <div className="flex items-center gap-2 text-green-400 font-bold">
                                    <div className="w-2 h-2 rounded-full bg-green-400 animate-pulse"></div>
                                    Online & Real-time
                                </div>
                                <p className="mt-4 text-[10px] text-slate-500 leading-relaxed uppercase tracking-widest font-bold">
                                    De TV-weergave link kan via het Admin paneel worden gekopieerd.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-8">
                            <SickRegistration 
                              entries={sickEntries} 
                              onChange={handleUpdateSick} 
                              onDelete={handleDeleteSick}
                ...
                {viewMode === 'SUBSTITUTES' && (
                    <SubstituteManagement substitutes={substitutes} onChange={handleUpdateSubstitutes} selectedWeek={currentWeek} />
                )}
                {viewMode === 'PROCESSING' && (
                    <ReplacementProcessing 
                        sickEntries={sickEntries} 
                        replacements={replacements} 
                        substitutes={substitutes}
                        onChange={handleUpdateReplacements}
